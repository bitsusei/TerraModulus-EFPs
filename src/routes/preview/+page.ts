...
export async function load({ fetch, url, parent }: PageLoadEvent) {
	if (url.searchParams.get("trial") !== null) {
		const id = url.searchParams.get("trial")!
		if ((await parent()).map[id] !== undefined) {
			const target = new URL("/efp/" + id, url)
